// Given a string of words separated by spaces, write a function that swaps the first and last letters of every word.

// You may assume that every word contains at least one letter, and that the string will always contain at least one word. 
// You may also assume that each string contains nothing but words and spaces, and that there are no leading, trailing, or repeated spaces.

function swap(text) {
  let array = text.split(" ");
  let returnArray = [];
  
  for (let i = 0; i < array.length; i++) {
    let splitArray = array[i].split("");
    
    let oldArray = splitArray.slice();
    
    for (let j = 0; j < splitArray.length; j++) {
      splitArray[0] = oldArray[oldArray.length - 1];
      splitArray[splitArray.length - 1] = oldArray[0];
    }
    
   returnArray.push(splitArray.join(""));
    
  }
  
  return returnArray.join(" ");
}
  
  // array.forEach(element => {
  //   element[0]
  // })
  
  // array.forEach(function(element) {
  //   oldArray.forEach(oldElement => {
  //   element[0] = oldElement[oldElement.length - 1];
  //   element[element.length - 1] = oldElement[0];
  //   });
  // });
// }

swap('Oh what a wonderful day it is');  // "hO thaw a londerfuw yad ti si"
swap('Abcde');                          // "ebcdA"
swap('a');                              // "a"